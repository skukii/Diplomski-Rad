...
from pupygrib.base import BaseSection, Message, ValueArray
...
class Edition1(Message):
    ...
    def get_values(self) -> ValueArray:
        values: ValueArray = self.pds._scale_values(self.bds._unpack_values())
        if self.bitmap:
            mask = ~np.array(self.bitmap.bitmap, dtype=bool)
            mvalues = np.empty(len(mask), dtype=float)
            mvalues[~mask] = values
            values = np.ma.array(mvalues, mask=mask)
        if self.gds:
    ...
    @cached_property
    def gds(self) -> gds.GridDescriptionSection | None:
    ...
    @cached_property
    def bitmap(self) -> bitmap.BitMapSection | None:
    ...
    def __getitem__(self, index: int) -> BaseSection | None:
